...
def main(input, output, encode, decode, fps, binarization, negative, averaging, ntiles, seekrange, gop, quality, batch):
    ...
    with ZipFile(input, 'r') as zip_ref:
        # Extraer el contenido del ZIP en el mismo directorio
        zip_ref.extractall(Path(input).parent)
    click.echo(f'Archivo ZIP descomprimido en {Path(input).parent}')

    # Ahora puedes trabajar con las imágenes extraídas del ZIP. Por ejemplo, puedes listarlas:
    image_files = list(Path(input).parent.glob('*.png'))
    click.echo(f'Imágenes extraídas del ZIP: {image_files}')

    # Visualizar la primera imagen (puedes ajustar según sea necesario)
    if image_files:
